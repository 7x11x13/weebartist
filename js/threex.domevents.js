...
THREEx.DomEvents	= function(camera, domElement)
{
	...
	this._$onTouchCancel	= function(){ _this._onTouchCancel.apply(_this, arguments);	};
	...
	this._domElement.addEventListener( 'touchcancel', this._$onTouchCancel	, false );
...
THREEx.DomEvents.prototype.destroy	= function()
{
	...
	this._domElement.removeEventListener( 'touchcancel'	, this._$onTouchCancel	, false );
...
THREEx.DomEvents.prototype.bind	= function(object3d, eventName, callback, useCapture)
{
	...
	for (const event of eventNames) {
		console.assert( THREEx.DomEvents.eventNames.indexOf(event) !== -1, "not available events:"+event );

		if( !this._objectCtxIsInit(object3d) )	this._objectCtxInit(object3d);
		var objectCtx	= this._objectCtxGet(object3d);	
		if( !objectCtx[event+'Handlers'] )	objectCtx[event+'Handlers']	= [];

		objectCtx[event+'Handlers'].push({
			callback	: callback,
			useCapture	: useCapture
		});
		
		// add this object in this._boundObjs
		if( this._boundObjs[event] === undefined ){
			this._boundObjs[event]	= [];	
		}
		this._boundObjs[event].push(object3d);
	}
}
...
THREEx.DomEvents.prototype._onTouchCancel	= function(event){ return this._onTouchEvent('touchcancel'	, event);	}
